...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, D: int, device: str = 'cpu', dropout: float = 0.1) -> None:
        ...
        self.Q_linears = nn.ModuleList([nn.Linear(self.D, self.d_k).to(device) for _ in range(self.num_heads)])  # SHOULD WE CALL to(device) FOR EVERY SINGLE LAYER?
        ...
        self.mha_linear = nn.Linear(self.D, self.D)  # (self.num_heads, 1) WRONG, NEED TO UNDERSTAND WHY
        # DO WE NEED THIS LINEAR LAYER AT ALL?
    ...
    def _attention_head(
        ...
        if mask is not None:
            assert attention_scores.shape == mask.shape, \
                f'The mask tensor must have shape {attention_scores.shape}, Got {mask.shape} instead.'
    ...
    def forward(
        ...
        Takes the learned query, key and value matrices from the previous encoder and feeds them into the current
        encoder, computing the attention weights and the attention per each head.
        ...
        :return: multi_head_attention. Dim: (batch_size, sequence_length, D) where D is the number of attention
                 ...
                 stacked and not concatenated. Dim: (num_heads, batch_size, sequence_length, sequence_length)
        """
        assert pre_query.shape == pre_key.shape == pre_value.shape, \
            f"query, key and value must have the same shape. Got shapes {pre_query.shape}, {pre_key.shape}, and " \
            f"{pre_value.shape} instead."
        ...
        attention_weights = torch.stack(attention_weights_per_head, dim=0).permute(0, 3, 1, 2)  # WHY IS THIS STACKED AND NOT CONCATENATED?
        multi_head_attention = self.mha_linear(self.dropout(multi_head_attention))
        return multi_head_attention, attention_weights  # WHY DO WE NEED TO KEEP TRACK OF THE ATTENTION WEIGHTS?
...
class PositionalEncoding(nn.Module):
    ...
    """
    def __init__(self, D: int, seq_length: int, dropout: float = 0.1) -> None:
        """
        :param D: the length of the input matrix (per token). For the first encoder layer, this is the embedding
               matrix. In the paper D = 512.
        ...
        self.dropout = nn.Dropout(dropout)
        pos = torch.arange(0, seq_length).unsqueeze(1).float()  # dim: (seq_length, 1)
        pos_enc = torch.zeros(seq_length, D)  # initializing the positional encoding matrix
        even_index = torch.arange(0, D, step=2).float()  # dim: (D/2)
        pos_enc[:, 0::2] = torch.sin(pos / pow(10000, even_index / D))
        pos_enc[:, 1::2] = torch.cos(pos / pow(10000, (even_index + 1) / D))
    ...
    def forward(self, embs: torch.Tensor) -> torch.Tensor:
        ...
        pos_enc = self.pos_enc.detach()  # detached because we don't want to learn weights (it's fixed, not a parameter)
        pos_enc_emb = self.dropout(embs + pos_enc)  # WHY WOULD WE NEED self.pos_enc[:, :x.shape[1]]
        return pos_enc_emb
